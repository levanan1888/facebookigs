<x-layouts.app.sidebar title="Thiết lập Website">
    <flux:main>
        <div class="p-6" x-data="{ name: '{{ addslashes(old('site_name', $setting->site_name ?? config('app.name'))) }}', logoPreview: '{{ $setting->logo_path ? asset('storage/'.$setting->logo_path) : '' }}', handleLogo(e){const f=e.target.files[0]; if(!f) return; this.logoPreview=URL.createObjectURL(f);} }">
